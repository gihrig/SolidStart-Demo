...
- **The hook is thin; the logic is a tested script.** `frontend/.vite-hooks/pre-push`
  ...
  `scripts/pre-push-scan.sh` with no arguments, letting git's stdin flow through. That
  wrapper runs the SAME gate `cgs scan` runs (`scripts/grype-scan.sh`), so the gate logic
  still lives in one place. The wrapper adds three things: it forces grype **offline** with
  `GRYPE_DB_AUTO_UPDATE=false` (cached DB, no network per push); it scans the **pushed
  refs** rather than the working tree (below); and on any failure it prints the next steps
  a developer needs (grype-scan.sh says which case failed; the wrapper says how to fix it —
  that advice is hook-specific, so grype-scan.sh, which CI shares, is untouched).
  `scripts/pre-push-scan.test.sh` proves all of it with a stub grype, so it needs no DB and
  does not flap; it runs in the `grype-scan` CI job.
- **It scans each pushed ref's committed SBOM, not the working tree (#143 code-review, P1).**
  git feeds a pre-push hook the refs being pushed on stdin
  (`<local-ref> <local-sha> <remote-ref> <remote-sha>`). The wrapper reads them and, for
  each non-deletion ref, scans that ref's committed SBOM via
  `git show <local-sha>:sbom.cdx.json` — so `git push origin other-branch` is gated on what
  is actually pushed, not on whatever happens to be checked out. A ref deletion (all-zero
  sha) is skipped; a ref with no `sbom.cdx.json` is skipped with a notice. With no refs on
  stdin (a direct/manual run, or the test), it falls back to the working-tree
  `sbom.cdx.json`. An earlier draft always scanned the working tree; Greptile flagged that a
  non-HEAD or multi-ref push could then pass or block on the wrong content.
...
- **The test exercises the tracked hook and the no-argument path (#143 code-review, P2).**
  An earlier draft only drove the wrapper with an explicit fixture path, so a broken hook
  file or the no-argument SBOM selection could have gone unnoticed. `pre-push-scan.test.sh`
  now has eight cases: offline env, finding+bypass hint, clean, tool-error+DB hint (direct
  mode); a pushed ref scanned, a deletion skipped, a ref with no SBOM skipped (stdin mode);
  and the tracked `frontend/.vite-hooks/pre-push` run as git runs it, asserting it gates the
  working-tree `sbom.cdx.json`. The stdin cases use a real sha from this repo (HEAD and the
  empty tree). Registration itself (`core.hooksPath`) is per-machine local config, so CI
  cannot assert it; the test covers the hook file and its dispatch, not the local config.
- **Validation.** The eight-case `pre-push-scan.test.sh` passes. End-to-end with the real
